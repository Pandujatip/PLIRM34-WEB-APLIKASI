...
const pwaBomDetailPanel = document.getElementById("pwa-bom-detail-panel");
const pwaBomDetailType = document.getElementById("pwa-bom-detail-type");
const pwaBomDetailTitle = document.getElementById("pwa-bom-detail-title");
const pwaBomDetailBody = document.getElementById("pwa-bom-detail-body");
...
const pwaBomStockResults = document.getElementById("pwa-bom-stock-results");
...
function getCarbonBrushMasterReferenceNames(references = []) {
  const carbonReferences = references.filter((item) =>
    isCarbonBrushSourceGroup(item.sourceGroup) || isCarbonBrushSourceGroup(item.category));
...
async function loadCarbonBrushEquipmentReference() {
  if (!carbonBrushEquipmentInput) {
    return carbonBrushEquipmentReferenceList;
  }

  const currentTypedValue = String(carbonBrushEquipmentInput.value || "").trim();
  ...
  try {
    if (backendState.available && backendState.sessionActive) {
      ...
      let sourceLabel = "Master Equipment";

      const cachedMasterReferences = getCarbonBrushMasterReferenceNames(backendState.masters.equipmentReferences);
      backendReferences = mergeCarbonBrushEquipmentReferences(backendReferences, cachedMasterReferences);
      ...
      if (backendState.sessionActive) {
        ...
        backendReferences = mergeCarbonBrushEquipmentReferences(backendReferences, getCarbonBrushMasterReferenceNames(allReferences));
      ...
      carbonBrushEquipmentReferenceList = mergeCarbonBrushEquipmentReferences(backendReferences);
      if (!carbonBrushEquipmentReferenceList.length) {
        throw new Error("Referensi carbon brush Master Equipment kosong");
      }
      ...
      } else if (currentTypedValue) {
        carbonBrushEquipmentInput.value = currentTypedValue;
      } else {
        carbonBrushEquipmentInput.value = "";
        selectedCarbonBrushEquipmentReference = "";
      }
      updateCarbonBrushEquipmentStatus(`Referensi carbon brush aktif: ${carbonBrushEquipmentReferenceList.length} item dari ${sourceLabel} kategori/source group carbon-brush.`);
    ...
    carbonBrushEquipmentInput.disabled = false;
    carbonBrushEquipmentInput.placeholder = "Master Equipment carbon-brush belum tersedia";
    carbonBrushEquipmentInput.value = currentTypedValue;
    selectedCarbonBrushEquipmentReference = "";
    hideCarbonBrushEquipmentResults();
    updateCarbonBrushEquipmentStatus("Referensi carbon brush wajib berasal dari Master Equipment kategori/source group carbon-brush. Tambahkan referensi di Manajemen User > Master Equipment.", true);
    renderPwaCarbonEquipmentOptions();
    return carbonBrushEquipmentReferenceList;
  }
}

async function ensureCarbonBrushEquipmentReferenceLoaded(options = {}) {
  const force = Boolean(options.force);
  if (!force && carbonBrushEquipmentReferenceList.length) {
    return carbonBrushEquipmentReferenceList;
  }
  if (carbonBrushEquipmentReferencePromise) {
    return carbonBrushEquipmentReferencePromise;
  }

  carbonBrushEquipmentReferencePromise = loadCarbonBrushEquipmentReference()
    .catch(() => carbonBrushEquipmentReferenceList)
    .finally(() => {
      carbonBrushEquipmentReferencePromise = null;
    });
  return carbonBrushEquipmentReferencePromise;
}

async function showCarbonBrushEquipmentResults(query = "") {
  await ensureCarbonBrushEquipmentReferenceLoaded();
  renderCarbonBrushEquipmentResults(String(query || ""));
}

async function showPwaCarbonEquipmentDropdown() {
  await ensureCarbonBrushEquipmentReferenceLoaded();
...
function analyzeServiceItem(item) {
  ...
  if (item.formType === "service-motor-mso") {
    ...
    const geDiagnostics = getMsoMotorGeDiagnostics(payload);
    ...
    if (geDiagnostics.afterCriticalCount > 0) {
      notes.push(`gE after masih kritis pada ${geDiagnostics.afterCriticalCount} kanal. Cek kualitas regrease dan kondisi bearing.`);
    } else if (geDiagnostics.afterWatchCount > 0) {
      notes.push(`gE after masih watchlist pada ${geDiagnostics.afterWatchCount} kanal. Pantau ulang titik yang sama pada inspeksi berikutnya.`);
    }
    if (geDiagnostics.dominantBefore?.value !== undefined && geDiagnostics.dominantAfter?.value !== undefined) {
      if (geDiagnostics.dominantAfter.value < geDiagnostics.dominantBefore.value) {
        notes.push(`gE membaik setelah regrease (${geDiagnostics.dominantBefore.value} -> ${geDiagnostics.dominantAfter.value}).`);
      } else {
        notes.push(`gE after belum turun dari before (${geDiagnostics.dominantBefore.value} -> ${geDiagnostics.dominantAfter.value}). Evaluasi regrease dan bearing.`);
      }
    }
...
function classifyMsoMotorGeValue(value) {
  if (value === null || value === undefined) {
    return "";
  }
  if (value > 4) {
...
function getMsoMotorGeChannelEntries(payload = {}, phase = "before") {
  const suffix = phase === "after" ? "After" : "Before";
  return msoMotorGeChannelDefinitions
    .map((channel) => {
      const value = parseMsoMotorNumeric(payload[`${channel.key}${suffix}`]);
      return {
        ...channel,
        phase,
        value,
        bucket: classifyMsoMotorGeValue(value),
      };
    })
...
function getMsoMotorGeValues(payload = {}, phase = "before") {
  return getMsoMotorGeChannelEntries(payload, phase).map((entry) => entry.value);
}
...
function getMsoMotorMaxGe(payload = {}, phase = "before") {
  const values = getMsoMotorGeValues(payload, phase);
  return values.length ? Math.max(...values) : null;
}
...
function getMsoMotorGeDiagnostics(payload = {}) {
  const beforeEntries = getMsoMotorGeChannelEntries(payload, "before");
  const afterEntries = getMsoMotorGeChannelEntries(payload, "after");
  ...
  const channelComparisons = msoMotorGeChannelDefinitions.map((channel) => {
    ...
    return {
      ...
      improved: before?.value !== undefined && after?.value !== undefined && after.value < before.value,
    };
  });
  return {
    beforeEntries,
...
function buildMsoMotorDetailSummary(payload = {}) {
  ...
  const maxGeAfter = geDiagnostics.dominantAfter?.value ?? null;
  return `Condition ${condition} | Temp max ${maxTemperature ?? "-"} | Vib before ${maxVibrationBefore ?? "-"} | gE after ${maxGeAfter ?? "-"}`;
}
...
function analyzeMsoMotorInspectionCadence(history) {
  ...
  for (let index = 1; index < sortedHistory.length; index += 1) {
    ...
    const previousBefore = getMsoMotorMaxGe(previousPayload, "before");
    const previousAfter = getMsoMotorMaxGe(previousPayload, "after");
...
function analyzeMsoMotorRegreaseEffectiveness(history) {
  const inspectionPairs = history.map((entry) => {
    ...
    const before = getMsoMotorMaxGe(payload, "before");
    const after = getMsoMotorMaxGe(payload, "after");
    return {
      ...
      stronglyImproved: before !== null && after !== null && before >= 2.1 && after < 2.1,
      unresolved: before !== null && after !== null && after >= before,
      highBefore: before !== null && before >= 2.1,
      lowAfter: after !== null && after < 2.1,
    };
...
function getMsoMotorHealthSnapshot(item) {
  ...
  const geDiagnostics = getMsoMotorGeDiagnostics(payload);
  const maxVibrationBefore = vibrationDiagnostics.dominantBefore?.value ?? null;
  const maxGeBefore = geDiagnostics.dominantBefore?.value ?? null;
  const maxGeAfter = geDiagnostics.dominantAfter?.value ?? null;
  ...
  if (geDiagnostics.afterCriticalCount > 0) {
    score -= 12 + ((geDiagnostics.afterCriticalCount - 1) * 4);
    notes.push(`gE after masih kritis pada ${geDiagnostics.afterCriticalCount} kanal. Dominan di ${geDiagnostics.dominantAfter?.label || "-"}.`);
  } else if (geDiagnostics.afterWatchCount > 0) {
    score -= Math.min(10, geDiagnostics.afterWatchCount * 3);
    notes.push(`gE after masih watchlist pada ${geDiagnostics.afterWatchCount} kanal.`);
  }
  if (maxGeAfter !== null && maxGeBefore !== null && maxGeAfter >= maxGeBefore) {
    score -= 10;
    notes.push(`gE after belum membaik dari before. Kanal dominan after ada di ${geDiagnostics.dominantAfter?.label || "-"} (${maxGeAfter}).`);
  } else if (maxGeAfter !== null && maxGeBefore !== null) {
    score += Math.min(6, Math.round((maxGeBefore - maxGeAfter) * 2));
    notes.push(`gE membaik setelah regrease (${maxGeBefore} -> ${maxGeAfter}).`);
  }
  if (recentBadCount >= 2) {
    score -= 12;
  ...
  if (regreaseEffect.consistentRecovery) {
    score += 6;
    notes.push(`Pola histori menunjukkan regrease cukup efektif: pada ${regreaseEffect.repeatHighBeforeLowAfterCount} inspeksi, gE before tinggi lalu turun rendah setelah regrease.`);
    notes.push("Artinya tindakan regrease memberi perbaikan nyata pada indikasi bearing, tetapi equipment tetap perlu diawasi dari trend vibrasi before.");
  } else if (regreaseEffect.unresolvedCount >= 2) {
    score -= 6;
    notes.push(`Pada ${regreaseEffect.unresolvedCount} inspeksi, gE after tidak turun dari before. Efektivitas regrease perlu dievaluasi ulang.`);
  } else if (regreaseEffect.pairCount === 0) {
    notes.push("Belum ada pair gE before/after yang cukup untuk menilai efektivitas regrease.");
  }
  ...
  if (cadence.reappearedAfterEffectiveRegreaseCount > 0) {
    ...
    notes.push(`Pada ${cadence.reappearedAfterEffectiveRegreaseCount} siklus, gE sempat membaik setelah regrease tetapi vibrasi before sudah naik lagi sebelum siklus inspeksi ${cadence.standardDays} hari berikutnya. Ini tanda interval kunjungan bisa terlalu longgar untuk equipment ini.`);
  }
  ...
  return {
    ...
    dominantGeBefore: geDiagnostics.dominantBefore,
    dominantGeAfter: geDiagnostics.dominantAfter,
    ...
    geBeforeCriticalCount: geDiagnostics.beforeCriticalCount,
    geBeforeWatchCount: geDiagnostics.beforeWatchCount,
    geAfterCriticalCount: geDiagnostics.afterCriticalCount,
    geAfterWatchCount: geDiagnostics.afterWatchCount,
    vibrationChannels: vibrationDiagnostics.channelComparisons,
    geChannels: geDiagnostics.channelComparisons,
    regreaseEffect,
    cadence,
...
function buildMsoMotorAnalyticsHtml(item) {
  ...
  const geTrendHtml = buildMsoMotorTrendSvg(trendHistory, [
    { label: "gE Before Max", getValue: (entry) => getMsoMotorMaxGe(entry.payload, "before"), color: "rgba(255,166,66,0.96)" },
    { label: "gE After Max", getValue: (entry) => getMsoMotorMaxGe(entry.payload, "after"), color: "rgba(115,224,169,0.96)" },
  ], {
    ariaLabel: "Grafik tren gE motor MSO",
    thresholds: [
      { value: 2.1, label: "Watch 2.1 gE", color: "rgba(255,210,79,0.75)" },
      { value: 4, label: "Critical 4 gE", color: "rgba(255,107,120,0.75)" },
    ],
  });
  ...
  if (latestSnapshot.geAfterCriticalCount > 0 || latestSnapshot.geAfterWatchCount > 0) {
    recommendations.push("gE after masih belum normal. Cek jumlah/teknik regrease, kondisi grease, dan indikasi awal bearing.");
  }
  if (latestSnapshot.maxGeAfter !== null && latestSnapshot.maxGeBefore !== null && latestSnapshot.maxGeAfter >= latestSnapshot.maxGeBefore) {
    recommendations.push("gE after belum turun dari before. Tinjau ulang metode regrease atau kebutuhan investigasi bearing lebih lanjut.");
  }
  if (latestSnapshot.regreaseEffect.consistentRecovery) {
    recommendations.push(`Pola histori menunjukkan setelah regrease, gE sering turun dari before tinggi menjadi after rendah (${latestSnapshot.regreaseEffect.repeatHighBeforeLowAfterCount} inspeksi).`);
    recommendations.push("Artinya regrease masih efektif sebagai tindakan korektif cepat, tetapi trend vibrasi before tetap dipakai untuk membaca kondisi mekanik utama.");
  }
  ...
  return `
    <section class="detail-card mso-health-card">
      <div class="mso-health-head">
        <div>
          ...
          <p>Penilaian cepat berdasarkan kondisi terbaru, temperatur, vibrasi before, serta perubahan gE before/after.</p>
        </div>
      ...
      <div class="detail-grid trend-summary-grid">
        ${buildDetailGridRows([
          ...
          ["gE before max", latestSnapshot.maxGeBefore ?? "-"],
          ["gE after max", latestSnapshot.maxGeAfter ?? "-"],
          ["Kanal gE after dominan", latestSnapshot.dominantGeAfter?.label || "-"],
          ["Status regrease", latestSnapshot.maxGeAfter === null ? "Belum ada data gE after" : "Evaluasi dari gE before/after"],
          ...
          ["gE after kritis", `${latestSnapshot.geAfterCriticalCount || 0} kanal`],
          ["gE after watch", `${latestSnapshot.geAfterWatchCount || 0} kanal`],
          ["Pair gE before/after", `${latestSnapshot.regreaseEffect.pairCount || 0} inspeksi`],
          ["gE before tinggi -> after rendah", `${latestSnapshot.regreaseEffect.repeatHighBeforeLowAfterCount || 0} inspeksi`],
    ...
    <section class="detail-card carbon-brush-trend-card">
      <div class="detail-modal-head compact-trend-head">
        <div>
          <h4>Trend Vibrasi</h4>
          <p>Vibrasi Motor MSO hanya dinilai dari nilai before. Grafik ini dipakai untuk membaca kenaikan getaran mekanik dari histori inspeksi.</p>
        </div>
      </div>
      ${vibrationTrendHtml}
    </section>
    <section class="detail-card carbon-brush-trend-card">
      <div class="detail-modal-head compact-trend-head">
        <div>
          <h4>Trend gE</h4>
          <p>gE memiliki nilai before dan after. Perbandingan ini dipakai untuk membaca efektivitas regrease pada indikasi bearing.</p>
        </div>
      </div>
      ${geTrendHtml}
    </section>
    ...
    <section class="detail-card">
      <h4>Ringkasan Kanal gE</h4>
      <div class="detail-grid">
        ${buildDetailGridRows(
          latestSnapshot.geChannels.flatMap((channel) => ([
            [`${channel.label} Before`, channel.beforeValue ?? "-"],
...
function renderPwaBomStockSearchResults(query) {
  if (!pwaBomStockResults) {
    return;
  }
  const results = searchMasterSparepartReferences(query);
  if (!results.length) {
    pwaBomStockResults.classList.remove("visible");
    pwaBomStockResults.innerHTML = "";
    return;
  }
  pwaBomStockResults.innerHTML = results.map((item) => `
    <button type="button" class="bom-stock-search-option" data-pwa-bom-stock-option="${escapeHtml(item.stockNo || "")}">
      <strong>${escapeHtml(item.stockNo || "-")}</strong>
      <span>${escapeHtml(item.materialDescription || "-")}</span>
      ${item.longText ? `<small>${escapeHtml(item.longText)}</small>` : ""}
    </button>
  `).join("");
  pwaBomStockResults.classList.add("visible");
}

function closePwaBomStockSearch() {
  if (pwaBomStockResults) {
    pwaBomStockResults.classList.remove("visible");
    pwaBomStockResults.innerHTML = "";
  }
...
function applySparepartReferenceToPwaBomForm(reference) {
  if (!reference) {
    return;
  }
  if (pwaBomStockNo) pwaBomStockNo.value = reference.stockNo || "";
  if (pwaBomMaterialDescription) pwaBomMaterialDescription.value = reference.materialDescription || "";
  if (pwaBomLongText) pwaBomLongText.value = reference.longText || reference.materialDescription || "";
  closePwaBomStockSearch();
}

function getPwaBomItemById(itemId, type = "general") {
  return type === "motor"
    ? getBomMotorItemsFromDom().find((entry) => entry.id === itemId)
    : getBomItemsFromDom().find((entry) => entry.id === itemId);
}

function openPwaBomDetail(item, type = "general") {
  if (!item || !pwaBomDetailPanel || !pwaBomDetailBody) {
    return;
  }
  const catalogItem = {
    ...item,
    catalogType: type,
    title: type === "motor"
      ? (item.materialDescription || item.manufacture || item.stockNo || "BOM Motor")
      : (item.part || item.materialDescription || item.stockNo || "BOM General"),
    unit: item.equipment || "-",
    quantity: type === "motor" ? (item.power || "1") : (item.qty || "-"),
    photoName: type === "motor" ? (item.motorPhoto || "") : (item.itemPhoto || ""),
    photoUrl: type === "motor" ? buildBomMotorPhotoPath(item.motorPhoto || "") : buildBomPhotoPath(item.itemPhoto || ""),
  };
  if (pwaBomDetailType) {
    pwaBomDetailType.textContent = type === "motor" ? "Detail BOM Motor" : "Detail BOM General";
  }
  if (pwaBomDetailTitle) {
    pwaBomDetailTitle.textContent = catalogItem.title || "-";
  }
  const photoHtml = catalogItem.photoName
    ? `<img src="${escapeHtml(catalogItem.photoUrl)}" alt="Foto ${escapeHtml(catalogItem.title)}" loading="lazy">`
    : `<span>${escapeHtml(String(catalogItem.title || "BM").slice(0, 2))}</span>`;
  pwaBomDetailBody.innerHTML = `
    <div class="pwa-bom-detail-hero">${photoHtml}</div>
    <div class="pwa-bom-detail-grid">
      <span>Equipment</span><strong>${escapeHtml(catalogItem.unit || "-")}</strong>
      <span>Jumlah / Power</span><strong>${escapeHtml(catalogItem.quantity || "-")}</strong>
      <span>No Stock SAP</span><strong>${escapeHtml(item.stockNo || "-")}</strong>
      <span>Deskripsi</span><strong>${escapeHtml(item.materialDescription || "-")}</strong>
      <span>Keterangan</span><strong>${escapeHtml(item.note || "-")}</strong>
    </div>
    ${item.longText ? `<p class="pwa-bom-detail-long">${escapeHtml(item.longText)}</p>` : ""}
  `;
  pwaBomDetailPanel.classList.remove("hidden");
  pwaBomFormPanel?.classList.add("hidden");
  window.setTimeout(() => pwaBomDetailPanel.scrollIntoView({ behavior: "smooth", block: "start" }), 20);
}

function closePwaBomDetail() {
  pwaBomDetailPanel?.classList.add("hidden");
}
...
function hydrateServiceForm(item) {
  ...
  if (item.formType === "service-motor-mv-carbon-brush") {
    ...
    if (carbonBrushEquipmentInput) {
      carbonBrushEquipmentInput.disabled = false;
      carbonBrushEquipmentInput.placeholder = "Ketik kode equipment dari Master Equipment carbon-brush";
    }
    void ensureCarbonBrushEquipmentReferenceLoaded();
    setCarbonBrushEquipmentValue(carbonBrushEquipmentName);
    updateCarbonBrushEquipmentStatus(`Mode edit aktif. Equipment tetap wajib dipilih dari Master Equipment carbon-brush.`);
    form.inspectionDate.value = String(payload.inspectionDate || "").slice(0, 10);
...
document.addEventListener("click", (event) => {
  ...
  if (serviceElectricalCarbonBrushForm && !serviceElectricalCarbonBrushForm.contains(event.target)) {
    hideCarbonBrushEquipmentResults();
  }
});
...
carbonBrushEquipmentInput?.addEventListener("input", (event) => {
  ...
  void showCarbonBrushEquipmentResults(query).then(() => {
    const loadedReference = findCarbonBrushEquipmentReferenceName(query);
    if (loadedReference) {
      selectedCarbonBrushEquipmentReference = loadedReference;
    }
  });
...
carbonBrushEquipmentInput?.addEventListener("focus", () => {
  void showCarbonBrushEquipmentResults(carbonBrushEquipmentInput.value);
});
...
carbonBrushEquipmentInput?.addEventListener("blur", () => {
  window.setTimeout(async () => {
    ...
    if (!matchedReference) {
      selectedCarbonBrushEquipmentReference = "";
      updateCarbonBrushEquipmentStatus("Pilih equipment carbon brush dari Master Equipment carbon-brush.", true);
    } else {
      selectedCarbonBrushEquipmentReference = matchedReference;
      carbonBrushEquipmentInput.value = matchedReference;
      updateCarbonBrushEquipmentStatus(`Equipment carbon brush dipilih dari Master Equipment carbon-brush. Total referensi aktif: ${carbonBrushEquipmentReferenceList.length} item.`);
    }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (formType === "service-motor-mv-carbon-brush") {
      ...
      if (!matchedCarbonBrushReference) {
        setSubmitNote(form, "Pilih equipment carbon brush dari Master Equipment kategori/source group carbon-brush terlebih dahulu.");
        showToast("Carbon Brush", "Equipment harus berasal dari Master Equipment carbon-brush.");
        return;
      }
      if (matchedCarbonBrushReference) {
...
pwaCompactShell?.addEventListener("click", async (event) => {
  ...
  const bomRow = target.closest(".pwa-bom-row");
  if (bomRow instanceof HTMLElement && !target.closest(".pwa-bom-actions")) {
    const itemId = bomRow.dataset.pwaBomId || "";
    const itemType = bomRow.dataset.pwaBomType || "general";
    const item = getPwaBomItemById(itemId, itemType);
    openPwaBomDetail(item, itemType);
    return;
  }

  if (target.closest("[data-pwa-bom-close-detail]")) {
    closePwaBomDetail();
    return;
  }
...
pwaCarbonEquipment?.addEventListener("input", () => {
  void showPwaCarbonEquipmentDropdown();
...
pwaBomStockNo?.addEventListener("input", () => {
  const reference = getSparepartReferenceByStockNo(pwaBomStockNo.value);
  if (reference) {
    applySparepartReferenceToPwaBomForm(reference);
    return;
  }
  renderPwaBomStockSearchResults(pwaBomStockNo.value);
});

pwaBomStockNo?.addEventListener("change", () => {
  applySparepartReferenceToPwaBomForm(getSparepartReferenceByStockNo(pwaBomStockNo.value));
});

pwaBomStockNo?.addEventListener("blur", () => {
  window.setTimeout(closePwaBomStockSearch, 260);
});
...
pwaCarbonForm?.addEventListener("submit", async (event) => {
  ...
  const matchedCarbonBrushReference = findCarbonBrushEquipmentReferenceName(selectedEquipment);
  if (!matchedCarbonBrushReference) {
    if (pwaCarbonFormNote) pwaCarbonFormNote.textContent = "Equipment belum ada di Master Equipment carbon-brush.";
    showToast("Carbon Brush", "Pilih equipment dari Master Equipment carbon-brush.");
    return;
  }

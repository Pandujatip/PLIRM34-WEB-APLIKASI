...
class OvertimeCalculationTests(unittest.TestCase):
    ...
    def test_overtime_deduction_for_sbg_and_smor(self):
        ...
        # 1. PT SBG & PT SMOR on day_off >= 8.0h -> deduction 1.0
        ...
        # 2. PT SBG & PT SMOR on day_off < 8.0h (e.g. 4.0h or 7.0h from 15:30-23:30) -> no deduction
        ...
        deduction, key, reason = calculate_overtime_deduction(
            "sbg", "day_off", 7.0, start_time="15:30:00", end_time="23:30:00"
        )
        self.assertEqual(deduction, 0.0)

        # 3. PT SBG & PT SMOR on workday continuing shift 1 -> 2 >= 8.0h -> deduction 1.0
        deduction, key, reason = calculate_overtime_deduction(
        ...
        # 4. PT SMOR on workday continuing shift 1 -> 2 with 7.0h (16:30-23:30) -> no deduction
        deduction, key, reason = calculate_overtime_deduction(
            "smor", "workday", 7.0, start_time="16:30:00", end_time="23:30:00", schedule_shift_code="1"
        )
        self.assertEqual(deduction, 0.0)

        # 5. PT SBG on workday continuing shift 2 -> 3 >= 8.0h (start ~23:00, end ~07:00) -> deduction 1.0
        deduction, key, reason = calculate_overtime_deduction(
        ...
        # 6. PT SMOR on regular overtime workday (e.g. 4 hours from 16:30 to 20:30) -> no deduction
        deduction, key, reason = calculate_overtime_deduction(
        ...
        # 7. PT SONAR on day_off or shift 1->2 (even if 8h) -> no deduction (remains standard)
...
assert after["summary"]["totalLiveHours"] == 18.0
...
assert hasan["monthLiveHours"] == 15.5
assert hasan["contractLiveHours"] == 15.5
assert hasan["annualRemainingHours"] == 368.5
assert hasan["groupType"] == "gangguan"
groups = {item["groupType"]: item for item in after["summary"]["groupTotals"]}
assert groups["gangguan"]["liveHours"] == 15.5
...
assert ranking[0]["contractLiveHours"] == 16.0
assert ranking[0]["annualRemainingHours"] == 368.0
...
assert admin_items[0]["contractLiveHours"] == 8.0

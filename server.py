...
class PLIRMRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_items_get(self, path: str, query: str = ""):
        ...
        # Add pagination support for all resources
        pagination = parse_pagination_params(query or "", default_limit=50, max_limit=200)
        all_items = list_items(resource_key)
        total = len(all_items)
        ...
        response = create_paginated_response(paginated_items, total, pagination['page'], pagination['limit'])
        
        # Add cache headers for GET requests
        self._send_json_cached(response, max_age=300, status=HTTPStatus.OK, public=False)
